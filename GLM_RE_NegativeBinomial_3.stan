...
model {
 ...
 if(PRIOR_ONLY ==0){
  for(i in 1:n){
   target += neg_binomial_2_log_lpmf(y[i]|beta[1] + gamma[year_id[n]] + delta[grid_id[n]] + beta[2]*salinity[i], phi);
}
}
}
...
for(i in 1:n){
ypred[i] = neg_binomial_2_log_rng(beta[1] + gamma[year_id[n]] + delta[grid_id[n]] + beta[2]*salinity[i], phi);
log_lik[i] = neg_binomial_2_log_lpmf(y[i]|beta[1] + gamma[year_id[n]] + delta[grid_id[n]] + beta[2]*salinity[i], phi);
}
}
